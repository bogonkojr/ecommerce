    ...
    <section class="section">
        <div class="card">
            ...
            <div class="col-md-4 text-center pl-3">
                <a href="{{ route('portal.users.create')}}" class="btn btn-success rounded-pill">New Entry</a>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                         ...
                         <tr>
                            ...
                            <td>
                                ...
                                <!-- @if($user->status)
                                <button class="btn btn-success toggle-status" data-id="{{$user->id}}">Active</button>
                                @else
                                <button class="btn btn-danger toggle-status" data-id="{{$user->id}}">Inactive</button>
                                @endif -->
                                <form action="{{ route('portal.users.destroy', $user->id) }}" method="POST" action="{{ url('destroy')}}">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <div><a href="{{ route('portal.users.edit', $user->id) }}" class="btn icon btn-primary"><i class="bi bi-pencil"></i></a></div>
                                </div>
                            </td>
                        </tr>
                    @endforeach
